...
HUB_SYSTEM_PROMPT = """Ты — AI-ассистент платформы «AI Craft Hub» для ремесленников,
...
def analyze_product_image(image_bytes: bytes, mime_type: str = "image/jpeg") -> dict[str, Any]:
    ...
    user_text = (
        "Ты помощник маркетплейса AI Craft Hub для ремесленников и малого бизнеса. "
...
def narrate_credit_assessment(
    *,
    ...
    user = (
        "Ты финтех-аналитик микрокредитов для ремесленников и самозанятых (AI Craft Hub).\n"
...
def _demo_chat_reply(messages: list[dict[str, str]]) -> str:
    ...
    return (
        "Привет! Это демо-ответ AI Craft Hub: в файле project/.env не задан OPENAI_API_KEY. "
